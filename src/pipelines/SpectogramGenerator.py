...
class SpectogramGenerator:
    ...
    def fit(self, X: list[AudioData], y=None):
        return self

    def transform(self, X: list[AudioData]):
        spectogram_data = []
        for audio_data in X:
            spectogram = gen_spectrogram(audio_data.audio_signal, audio_data.sample_rate)
            spectogram_data.append(spectogram)
